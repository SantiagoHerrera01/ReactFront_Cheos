  ...
  <div className="fixed inset-0 z-[60] bg-black/30 backdrop-blur-sm flex items-center justify-center p-4" onClick={onClose}>
    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm border border-gray-100 overflow-hidden" onClick={e => e.stopPropagation()}>
      ...
      <div className="flex gap-2 px-5 pb-5">
        <button onClick={onClose} className="flex-1 py-2.5 rounded-xl text-sm font-medium border border-gray-200 text-gray-600 hover:bg-gray-50 transition">Cancelar</button>
        <button onClick={onConfirm} className="flex-1 py-2.5 rounded-xl text-sm font-semibold bg-red-500 text-white hover:bg-red-600 transition">Eliminar</button>
      </div>
    </div>
  </div>
)
...
const ProductForm = ({ product, onSave, onClose }) => {
  ...
  const [selectedImages, setSelectedImages] = useState(isEdit ? (product.images || []) : [])
  ...
  const submit = async () => {
    ...
    try {
      const payload = {
        ...
        price: parseFloat(form.price), weight: parseInt(form.weight),
        stock: parseInt(form.stock), category: form.category,
        images: selectedImages, is_featured: form.is_featured,
    ...
    <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-end sm:items-center justify-center p-0 sm:p-6" onClick={onClose}>
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-5 sm:px-6 py-5 space-y-4">
          ...
          <div>
            <GalleryImageSelector selectedImages={selectedImages} onImagesChange={setSelectedImages} token={token} singleSelect={false} />
        ...
        <div className="flex gap-2 px-5 sm:px-6 py-4 border-t border-gray-100 flex-shrink-0">
          <button onClick={onClose} className="flex-1 py-2.5 rounded-xl text-sm font-medium border border-gray-200 text-gray-600 hover:bg-gray-50 transition">Cancelar</button>
          <button onClick={submit} disabled={loading}
            className="flex-1 py-2.5 rounded-xl text-sm font-semibold bg-gray-900 text-white hover:bg-gray-700 transition disabled:opacity-40 flex items-center justify-center gap-2">
    ...
    <div className={`bg-white border rounded-xl overflow-hidden transition-all hover:shadow-md hover:-translate-y-px group ${!product.is_active ? 'opacity-60' : 'border-gray-200'}`}>
      <div className="relative h-40 sm:h-44 w-full overflow-hidden bg-gray-100">
        ...
        {product.stock > 0 && product.stock <= 10 && (
          <div className="absolute bottom-2 left-2">
            <span className="bg-red-600 text-white text-[10px] font-bold px-2 py-0.5 rounded-md shadow">Pocas unidades</span>
          </div>
        )}
        {product.is_featured && (
        ...
        {/* Acciones — visibles con tap/hover */}
        <div className="absolute top-2 right-2 flex gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
          <button onClick={() => onEdit(product)} className="bg-white/90 hover:bg-white p-1.5 rounded-lg shadow transition" title="Editar"><Edit3 size={15} className="text-gray-700" /></button>
          <button onClick={() => onStockEdit(product)} className="bg-white/90 hover:bg-white p-1.5 rounded-lg shadow transition" title="Editar stock"><Package size={15} className="text-gray-700" /></button>
          <button onClick={() => onDelete(product)} className="bg-white/90 hover:bg-red-500 p-1.5 rounded-lg shadow transition" title="Eliminar"><Trash2 size={15} className="text-gray-700 group-hover:text-white" /></button>
        </div>
      </div>
      ...
      <div className="p-3 sm:p-3.5">
        ...
        <div className="flex items-center justify-between">
          <p className="font-bold text-gray-900 text-sm sm:text-base">{fmt(product.price)}</p>
          <button onClick={() => onStockEdit(product)}
        ...
        <div className="flex gap-1.5 mt-3 pt-3 border-t border-gray-100">
          <button onClick={() => onToggleActive(product)}
            className={`flex-1 flex items-center justify-center gap-1 sm:gap-1.5 py-1.5 rounded-lg text-[11px] font-medium border transition
...
const StatsBar = ({ products }) => {
  ...
  const stats = [
    { label: 'Total',       value: total,    icon: Package,       color: 'text-gray-700',    bg: 'bg-gray-100' },
    ...
    { label: 'Crítico',     value: criticos, icon: TrendingDown,  color: 'text-orange-500',  bg: 'bg-orange-50' },
  ...
  return (
    <div className="grid grid-cols-3 sm:grid-cols-5 gap-2 sm:gap-3 mb-4 sm:mb-5">
      {stats.map(({ label, value, icon: Icon, color, bg }) => (
        <div key={label} className="bg-white border border-gray-200 rounded-xl px-3 sm:px-4 py-2.5 sm:py-3 flex items-center gap-2 sm:gap-3">
          <div className={`w-7 sm:w-8 h-7 sm:h-8 rounded-lg ${bg} flex items-center justify-center flex-shrink-0`}>
            <Icon size={13} className={color} />
          </div>
          <div className="min-w-0">
            <p className="text-base sm:text-lg font-bold text-gray-900 leading-tight">{value}</p>
            <p className="text-[9px] sm:text-[10px] text-gray-400 truncate">{label}</p>
          </div>
        </div>
      ))}
    </div>
...
export default function InventoryPanel() {
  ...
  const handleDelete = async () => {
    try {
      const res = await fetch(`${API_BASE}/products/${deleteTarget.id}`, { method: 'DELETE', headers: { Authorization: `Bearer ${token}` } })
  ...
  const handleToggleActive = async (product) => {
    try {
      ...
      const res = await fetch(`${API_BASE}/products/${product.id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` }, body: JSON.stringify({ is_active: newVal }) })
  ...
  const handleToggleFeatured = async (product) => {
    try {
      ...
      const res = await fetch(`${API_BASE}/products/${product.id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` }, body: JSON.stringify({ is_featured: newVal }) })
      if (!res.ok) throw new Error('Error')
  ...
  const clearFilters = () => { setFilterCat(''); setFilterStock(''); setSearch(''); setSearchInput('') }
  const hasFilters   = filterCat || filterStock || search
  ...
  return (
    <div className="space-y-4 sm:space-y-5">
      ...
      <div className="space-y-3">
        ...
        <div className="flex gap-2 items-center">
          <form onSubmit={handleSearchSubmit} className="relative flex-1 min-w-0">
            ...
            <input
              ...
              className="pl-9 pr-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-1 focus:ring-gray-800 focus:border-gray-800 w-full transition-all"
            />
          </form>
          ...
          <button
            ...
            className={`sm:hidden flex items-center gap-1.5 px-3 py-2 rounded-lg text-sm border transition flex-shrink-0
              ${hasFilters ? 'border-coffee text-coffee bg-coffee/5' : 'border-gray-200 text-gray-500'}`}
          >
            <ArrowUp size={13} className={showFilters ? 'rotate-180' : ''} style={{ transition: 'transform .2s' }} />
            Filtros {hasFilters ? '·' : ''}
          </button>

          <button
            onClick={() => setEditProduct(false)}
            className="flex items-center gap-1.5 px-3 sm:px-4 py-2 rounded-lg text-sm font-semibold bg-gray-900 text-white hover:bg-gray-700 transition shadow-sm flex-shrink-0"
          >
            <Plus size={14} />
        ...
        {/* Fila 2: filtros — siempre visibles en sm+, toggle en móvil */}
        <div className={`flex flex-wrap gap-2 ${showFilters ? 'flex' : 'hidden sm:flex'}`}>
          <select value={filterCat} onChange={e => setFilterCat(e.target.value)}
            className="border border-gray-200 rounded-lg px-3 py-2 text-sm text-gray-600 focus:outline-none focus:ring-1 focus:ring-gray-800 bg-white flex-1 sm:flex-none min-w-0">
      ...
      <div className="flex items-center gap-1 text-[11px] text-gray-400 flex-wrap">
        ...
        {[['name','Nombre'],['price','Precio'],['stock','Stock'],['category','Cat.']].map(([f, l]) => (
          <button key={f} onClick={() => handleSort(f)}
            className={`flex items-center gap-1 px-2 sm:px-2.5 py-1 rounded-md border transition ${sortBy === f ? 'border-gray-300 text-gray-700 bg-gray-50' : 'border-transparent text-gray-400 hover:text-gray-600'}`}>
            {l} <SortIcon field={f} />
          </button>
        ))}
        <span className="ml-auto text-gray-400">
          {filtered.length} producto{filtered.length !== 1 ? 's' : ''}
          {(filterCat || filterStock) && ` · filtrado`}
        </span>
      </div>
      ...
      {/* Modals */}
      {editProduct !== null && (
        <ProductForm product={editProduct || null} onSave={handleSaveProduct} onClose={() => setEditProduct(null)} />
      )}
      {deleteTarget && (
        <DeleteDialog product={deleteTarget} onConfirm={handleDelete} onClose={() => setDeleteTarget(null)} />
      )}
      {stockTarget && (
        <StockEditor product={stockTarget} onSave={handleSaveStock} onClose={() => setStockTarget(null)} />
      )}
    </div>
  )
}