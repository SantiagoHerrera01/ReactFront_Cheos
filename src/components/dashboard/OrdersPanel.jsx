    ...
    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm border border-gray-200 overflow-hidden" onClick={e => e.stopPropagation()}>
      <div className="px-5 pt-5 pb-4">
        ...
        <p className="text-sm text-gray-500">
          ¿Confirmas la cancelación del pedido <span className="font-mono font-bold text-gray-800">{order.order_number}</span>? Esta acción no se puede deshacer.
        </p>
      </div>
...
const Pagination = ({ page, totalPages, total, onPage }) => {
  ...
  return (
    <div className="flex flex-col sm:flex-row items-center justify-between gap-2 px-5 py-3.5 border-t border-gray-100">
      ...
      <div className="flex items-center gap-1">
        <button onClick={() => onPage(page - 1)} disabled={page === 1}
          className="flex items-center gap-1 px-2.5 py-1.5 rounded-md text-[11px] font-medium border border-gray-200 text-gray-400 hover:border-gray-400 hover:text-gray-700 transition-all disabled:opacity-30 disabled:cursor-not-allowed">
          <ChevronLeft size={12} /> Ant.
        </button>
        {list.map((p, i) => p === '…'
          ? <span key={`e${i}`} className="w-7 text-center text-[11px] text-gray-400">…</span>
          : <button key={p} onClick={() => onPage(p)}
              className={`w-7 h-7 rounded-md text-[11px] font-bold transition-all ${p === page ? 'bg-coffee text-white' : 'text-gray-400 hover:text-gray-700 hover:bg-gray-100'}`}>
              {p}
            </button>
        )}
...
const OrdersTable = ({ statusGroup, updatingId, handleStatusChange, approvePayment, cancelOrder, setSelectedOrder, token, apiBase, onOrderUpdated }) => {
  ...
  const handleCancel = async () => {
    ...
    await cancelOrder(cancelTarget, updateLocal)
    setCancelTarget(null)
  }

  if (loading) return (
    <div className="bg-white border border-gray-200 rounded-xl py-10 mb-5">
      <CoffeeLoader variant="grinder" message="Cargando pedidos..." size="sm" />
    </div>
  )
  if (error) return <p className="text-red-500 p-4">{error}</p>
  if (!orders.length) return (
    <div className="bg-white border border-gray-200 rounded-xl py-12 text-center text-gray-400 text-sm italic mb-5">
            ...
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                {['Orden', 'Cliente', 'Total', 'Estado', 'Pago', 'Fecha', 'Acciones'].map(h => (
                  <th key={h} className="px-4 py-3 text-left text-[9px] font-bold tracking-[0.14em] uppercase text-gray-400 whitespace-nowrap">{h}</th>
                ))}
              </tr>
            </thead>
                ...
                return (
                  <tr key={order.id} className="hover:bg-gray-50/70 transition-colors align-top">
                    <td className="px-4 py-3.5">
                      <span className="font-mono font-bold text-[11px] text-coffee tracking-wider whitespace-nowrap">{order.order_number}</span>
                    </td>
                    <td className="px-4 py-3.5 text-gray-700 font-medium whitespace-nowrap">{order.customer_name || '—'}</td>
                    <td className="px-4 py-3.5 font-bold text-gray-900 whitespace-nowrap">${order.total?.toLocaleString('es-CO')}</td>
                        ...
                        {nextStatus && !isFinished && (
                          <div className="relative">
                            <select
                              ...
                              value=""
                              onChange={e => { if (e.target.value) handleStatusChange(order, e.target.value, updateLocal) }}
                              className={[
                                'w-full appearance-none pl-2.5 pr-7 py-1.5 rounded-md border text-[11px] font-medium outline-none transition-all bg-white',
                                nextBlocked ? 'border-gray-200 text-gray-300 cursor-not-allowed' : 'border-gray-300 text-gray-600 hover:border-gray-400 cursor-pointer',
                    ...
                    <td className="px-4 py-3.5">
                      <div className="flex flex-col gap-0.5">
                        ...
                        <span className="text-[9px] text-gray-400 uppercase tracking-wider whitespace-nowrap">
                          {PM_LABELS[order.payment_method] || order.payment_method}
                        </span>
                      </div>
                    </td>

                    <td className="px-4 py-3.5 text-[11px] text-gray-400 whitespace-nowrap">{dateStr}</td>
                    ...
                    <td className="px-4 py-3.5">
                      <div className="flex flex-col gap-1.5">
                        {needsApproval && (
                          <button
                            disabled={isUpdating}
                            onClick={() => approvePayment(order, updateLocal)}
                            className="inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-md text-[11px] font-semibold border border-emerald-200 text-emerald-600 bg-emerald-50 hover:bg-emerald-100 transition-all disabled:opacity-40 whitespace-nowrap"
                          >
                            ✓ Confirmar pago
                          </button>
                        )}
                        <button
                          ...
                          className="inline-flex items-center px-2.5 py-1.5 rounded-md text-[11px] font-medium border border-gray-200 text-gray-500 hover:border-gray-400 hover:text-gray-700 transition-all whitespace-nowrap"
                        >
                          Ver detalles
                        </button>
                        {canCancel && (
                          <button
                            ...
                            className="inline-flex items-center px-2.5 py-1.5 rounded-md text-[11px] font-medium border border-red-200 text-red-500 hover:bg-red-50 transition-all disabled:opacity-40 whitespace-nowrap"
                          >
                            Cancelar
                          </button>
                        )}
...
const OrderModal = ({ order, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/30 backdrop-blur-sm flex items-end sm:items-center justify-center z-50" onClick={onClose}>
      <div
        className="bg-white w-full sm:max-w-md rounded-t-2xl sm:rounded-2xl overflow-hidden flex flex-col border border-gray-200 shadow-2xl"
      ...
      >
              ...
              <div className="bg-gray-50 border border-gray-200 rounded-xl overflow-hidden divide-y divide-gray-100">
                {rows.map(([label, value]) => (
                  <div key={label} className="flex justify-between items-center px-3.5 py-2.5 text-sm gap-4">
                    ...
                    <span className={`font-semibold text-right truncate ${label === 'Estado' && order.payment_status === 'APPROVED' ? 'text-emerald-600' : 'text-gray-800'}`}>
                      {value || '—'}
                    </span>
                  </div>
                ))}
        ...
        <div className="px-5 py-3.5 border-t border-gray-100 flex-shrink-0">
          <button onClick={onClose} className="w-full py-2.5 rounded-xl text-sm font-semibold border border-gray-200 text-gray-700 bg-gray-50 hover:bg-gray-100 transition-colors">
            Cerrar
          </button>
        </div>
      </div>
